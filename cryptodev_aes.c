...
static int test_aes(int cfd)
{
	...
	uint8_t *buffer=malloc(1e7);
	size_t count = fread(buffer, 1, 1e7, meat);
	printf("%u bytes read\n", count);
...
printf("Read, starting timer.\n");
	...
	if (ctx.alignmask)
		buffer = (uint8_t *)(((unsigned long)buffer + ctx.alignmask) & ~ctx.alignmask);
	else
		buffer = buffer;

	memset(iv1, 0x0, sizeof(iv1));

	aes_encrypt(&ctx, iv1, buffer, buffer, count);
